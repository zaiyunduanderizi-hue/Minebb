...
const computeRetryDelay = (attempt, responseTtl, headerValue) => {
    ...
    return Math.min(MAX_RETRY_DELAY_MS, Math.max(computed, hintDelay));
};
...
const isAdapterError = (error) => Boolean(error && typeof error === "object" && "code" in error);
const postJson = async (path, body, { timeoutMs = 15_000, tokenId } = {}) => {
    ...
    while (true) {
        ...
        try {
            ...
            throw createError("NETWORK", error?.message ?? "Network error", {
                details: error,
            });
        }
        finally {
...
const toTimeseries = (symbol, market, timeframe, rows) => {
    const points = rows
        .reduce((acc, row) => {
        const t = parseTimestamp(row.date ??
            row.time ??
            row.timestamp ??
            row.t ??
            row.tradeTime ??
        ...
        const v = parseNumeric(row.volume ??
            row.v ??
            row.turnoverVolume ??
            row.amount ??
            row.turnover);
...
const toQuote = (symbol, market, raw) => {
    const price = parseNumeric(raw.price ?? raw.close ?? raw.c ?? raw.last) ?? 0;
    const change = parseNumeric(raw.change ?? raw.chg ?? raw.delta);
    const changePct = parseNumeric(raw.changePct ?? raw.chgPct ?? raw.pctChange ?? raw.percent);
    const ts = parseTimestamp(raw.time ?? raw.timestamp ?? raw.ts ?? raw.date ?? raw.lastTime) ?? Date.now();
    return parseQuote({
        symbol,
        market,
        price,
...
const toSearchResults = (rows) => {
    ...
    for (const row of rows) {
        ...
        const market = row.market ??
            (row.areaCode?.toUpperCase() === "HK" ? "HK" : "CN");
        const name = row.name ?? row.cnName;
        const entry = {
            symbol,
            market,
            ...(name ? { name } : {}),
